...
export function GalleryGrid({ images }: { images: GalleryImage[] }) {
  ...
  return <>
    <div className="gallery-collage">{visible.map((image, index) => <button className={`gallery-collage-tile gallery-collage-${index}`} key={image.src} onClick={() => setActive(page * perPage + index)}><img src={image.src} alt={image.alt} loading="lazy" decoding="async" /><span>{image.label}</span><i>View story</i><b className="gallery-mark">SINZA</b></button>)}</div>
    ...
    {active !== null && <div className="gallery-lightbox" role="dialog" aria-modal="true" aria-label="Gallery image" onClick={() => setActive(null)}><button className="lightbox-close" onClick={() => setActive(null)} aria-label="Close image"><X /></button><button className="lightbox-prev" onClick={(event) => { event.stopPropagation(); previous() }} aria-label="Previous image"><ChevronLeft /></button><img src={images[active].src} alt={images[active].alt} onClick={(event) => event.stopPropagation()} /><button className="lightbox-next" onClick={(event) => { event.stopPropagation(); next() }} aria-label="Next image"><ChevronRight /></button><p>{images[active].label}</p></div>}
  </>
}
